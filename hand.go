...
func (hr HandResult) HandScore() []int {
	...
	for _, c := range hr.Cards {
		score = append(score, c.(libcard.SuitRankCard).Rank)
	}
	return score
}
...
func Result(hand libcard.Deck) (result HandResult) {
	...
	for i := 0; i < 4; i++ {
		...
		if ok && (result.Category < CATEGORY_STRAIGHT_FLUSH ||
			cards[0].(libcard.SuitRankCard).Rank >
				result.Cards[0].(libcard.SuitRankCard).Rank) {
...
// Finds a multiple of a rank of libcard
